...
Protester::Protester(StudentWorld* world, int imageID, int hp)
    ...
    m_stunTicks(0), m_ticksSincePerpendicularTurn(200)
{
...
void Protester::doSomething()
{
    ...
    m_ticksToWait = std::max(0, 3 - (int)getWorld()->getLevel() / 4);
    m_ticksSincePerpendicularTurn++;

    // 2. Leaving State
    if (m_leaving) {
        m_ticksToWait = 0; // Move fast
        if (getX() == 60 && getY() == 60) {
            setDead();
            return;
        }
    ...
    if (getDistanceTo(getWorld()->getPlayer()) <= 4.0 && isFacingPlayer()) {
        ...
        getWorld()->getPlayer()->decHP(2); // Directly annoy player
        m_ticksToWait = 15;
        return;
    }
...
void ActivatableObject::doSomething()
{
    ...
    if (m_pickupableByPlayer) {
        if (getDistanceTo(getWorld()->getPlayer()) <= 3.0) {
            ...
            doEffect();
            return; // Important: Return so we don't double dip or crash
        }
...
void GoldNugget::doSomething() {
    ActivatableObject::doSomething(); // Handle player pickup first
    if (!isAlive()) return;
...
void Squirt::doSomething() {
    ...
    // Annoy Protesters
    // If we annoy someone, we die immediately to prevent multi-hit
